...
SOURCES = [
    *[(t, "train", ECH / f"data/ribocode_per_tissue/{t}/{t}_pre_config.txt") for t in
      ("Brain", "ES", "Fat", "Fibroblast", "HA_EC", "HCAEC", "Hepatocytes", "HUVEC", "VSMC")],
    ("Ruiz-Orera (human)", "heldout", ROOT / "data/heldout_psites/human_ruizorera/human_ruizorera_pre_config.txt"),
    ("Wang liver (mouse)", "heldout", ROOT / "data/heldout_psites/mouse_wang_liver/mouse_wang_liver_pre_config.txt"),
    ("GSE120762 NT (mouse)", "heldout", ROOT / "data/heldout_psites/mouse_gse120762_nt/mouse_gse120762_nt_pre_config.txt"),
    ("GSE120762 LPS (mouse)", "heldout", ROOT / "data/heldout_psites/mouse_gse120762_lps/mouse_gse120762_lps_pre_config.txt"),
    ("GSE155087 T-cell (mouse)", "heldout", ROOT / "data/heldout_psites/mouse_gse155087_tcell/mouse_gse155087_tcell_pre_config.txt"),
...
ROLE_COLOR = {"train": "#2f6f9f", "heldout": "#c1663a", "pgx": "#4a8c5f"}
...
def main():
    ...
    order = sorted(data, key=lambda d: (["train", "heldout", "pgx"].index(d["role"]), -d["f0_frac"]))
    ...
    for d in order:
        print(f"{d['label']:<26}{d['role']:<10}{d['n_libraries']:>5}{d['mode_read_length']:>9}"
              f"{d['f0_frac'] * 100:>7.1f}%{str(d['psite_offsets']):>12}{d['psites_at_cds']:>14,}")
