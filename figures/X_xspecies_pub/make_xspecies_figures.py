...
OUT = NEW / "figures" / "X_xspecies_pub"
...
def figure2():
    ...
    plt.close(fig)
    PROV["source"]["Fig2"] = [str(src_cls.relative_to(NEW)), str(src_all.relative_to(NEW))]
...
def figure4():
    ...
    plt.close(fig)
    PROV["source"]["Fig4"] = [str(src_a.relative_to(NEW)), str(src_b.relative_to(NEW))]
...
def figure1():
    ...
    reg = tsv(src)
    uni = {}
    for p, arm in ARM.items():
        f_ = NEW / "data" / "xspecies_refs" / f"{arm}_universe.tsv"
        if f_.exists():
            rs = tsv(f_)
            uni[p] = (len(rs), sum(1 for r in rs if r["biotype"] == "protein_coding"))
    ...
    plt.close(fig)
    PROV["source"]["Fig1"] = [
        str(src.relative_to(NEW)), "data/xspecies_refs/<arm>_universe.tsv",
...
def figure3():
    ...
    plt.close(fig)
    PROV["source"]["Fig3"] = [str(src.relative_to(NEW)),
...
if __name__ == "__main__":
    ...
    PROV["values"] = v
    (OUT / "X_xspecies_pub_values.json").write_text(json.dumps(PROV, indent=1))
