...
stronger point than "global inflates by 10x" because it cannot be dismissed as conservative.
  ...
  (b) Why it happens: score distributions of final-recipe targets, novel targets and novel decoys for
...
MODEL = "mamba4"
FDR = 0.01
# Panel (b) demo. This WAS ("A549", "null_atg") -- 375 novel peptides at global FDR vs 10 at
# class-specific, a 37.5x inflation, which was this panel's entire argument. A549 was removed
# from the project 2026-08-15, and the largest discrepancy among the remaining HLA-I sets is
# 2.3x (HBL1/cpat, 7 vs 3 peptides). HBL1/null_atg is used instead: its ratio is smaller
# (1.9x) but it has the largest absolute gap (39 vs 20), so the mechanism is still visible.
# THE HEADLINE MUST BE RESCOPED ACCORDINGLY -- see this figure's FIGURE_DATA_INPUTS.md.
DEMO = ("HBL1", "null_atg")
...
def main():
    ...
    if demo_rows is not None:
        ...
        ax.hist(canon, bins=bins, density=True, color="#2f6f9f", alpha=0.5, label="final-recipe targets")
    ...
    # This read "(A549, tryptic)" and kept RENDERING after A549 was removed from DATASETS, so the
    # panel labelled a region for a dataset it no longer plots. The over-reporting region is still
    # populated (HBL-1/CPAT, 7 vs 3), just far smaller than A549's 375-vs-10.
    ax.text(lo * 1.15, ax.get_ylim()[1] * 0.55, "global OVER-reports", fontsize=7.5,
            color="#c0392b")
