...
(d) TOTAL  -- change in TOTAL unique peptides (canonical + novel) vs the gencode-only baseline. The
              number an experimentalist takes home. (a) and (b) can both favour an arm that
              nonetheless leaves you with fewer peptides than not searching a novel DB at all, and
              (b)'s dPSM is a PSM count, not a peptide count. Added 2026-08-14 under the standing
              rule that all proteomics work reports absolute totals, including where they disfavour
              the model.
...
DATASETS = [("HBL1", "HBL-1"), ("DoHH2", "DoHH2"), ("SUDHL4", "SU-DHL-4")]
# (arm key, display, colour). The null_nc arm is GONE with A549: near-cognate nulls existed only for
# the tryptic dataset, because nonspecific digestion of a ~3M-sequence near-cognate null exceeds
# MSFragger's ~2e9 peptide cap. Removing A549 therefore removes an ARM, not just a column.
...
def main():
    ...
    fig, axes = plt.subplots(1, 4, figsize=(19.8, 5.0))
    ...
    panels = [
        ("a", "GAIN", "novel peptides per 1,000 DB seqs", "log",
         lambda a, _all: (a["novel_peptides"] / a["db_novel_seqs"] * 1000)
         if a["db_novel_seqs"] else np.nan),
        ("b", "COST", "change in GENCODE PSMs vs baseline", "symlog",
         lambda a, _all: float(a["d_gencode_psms"])),
        ("c", "DRIVER", "novel DB sequences (decoy load)", "log",
         lambda a, _all: float(a["db_novel_seqs"])),
        # Needs the gencode baseline arm, which is why every fn takes the full arms dict.
        ("d", "TOTAL", "change in TOTAL unique peptides vs baseline", "symlog", lambda a, _all:
         float(a["d_gencode_peptides"] + a["novel_peptides"]
               - _all.get("gencode", {}).get("novel_peptides", 0))),
    ]
    n_arm = len(ARMS)
    w = 0.8 / (n_arm * 2)
    ...
    for ax, (tag, title, ylab, scale, fn) in zip(axes, panels):
        ...
        for i, (arm, disp, col) in enumerate(ARMS):
            for j, (mdl, hatch) in enumerate((("mamba4", None), ("attn", "///"))):
                ...
                for k, (key, _) in enumerate(DATASETS):
                    ...
                    v = fn(a, data[mdl].get(key, {}))
                    xs.append(k + (i * 2 + j) * w - 0.4 + w / 2)
                    vals.append(v)
                    if tag in ("b", "d") and v == 0:
                        zero_xs.append(xs[-1])
                if vals:
        ...
        if tag in ("b", "d"):
            ...
            if zero_xs:
                ...
                ax.text(0.02, 0.97,
                        "◆  0 = measured, no canonical cost" if tag == "b"
                        else "◆  0 = measured, no net change",
                        transform=ax.transAxes, ha="left", va="top",
                        fontsize=7.5, color="#2C6FBB")
            ax.set_ylabel(ylab + ("\n(negative = net LOSS vs not searching)" if tag == "d"
    ...
    vals = {mdl: {k: {arm: {"density_per_1k": (a["novel_peptides"] / a["db_novel_seqs"] * 1000)
                           ...
                           "d_gencode_peptides": a["d_gencode_peptides"],
                           "d_total_peptides": (a["d_gencode_peptides"] + a["novel_peptides"]
                                                - arms.get("gencode", {})
                                                    .get("novel_peptides", 0))}
