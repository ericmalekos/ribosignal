...
> **SUPERSEDED 2026-08-15: A549 was archived and removed from every figure.** Figure 2 is now built
> from **4 HLA-I datasets** (HBL-1, SU-DHL-4, DoHH2, THP-1) = **16 points**, median 93.6x unchanged.
> With the only tryptic dataset gone, **Figure 2 is an HLA-I immunopeptidome result and must not be
> captioned as a whole-proteome one.** Panel (b)'s CPAT/CPC2 comparison lost the substrate on which
> CPAT/CPC2 won, and panel (c) lost the `null_nc` arm entirely. See each panel's own
> `FIGURE_DATA_INPUTS.md` and `proteogenomics/data/_archive_a549_2026_08_15/`.
