...
def main():
    ...
    ap.add_argument("--model", default=MODEL, choices=["mamba4", "attn"],
                    help="mamba4 = historical default; attn = the poster's transformer")
    a_ = ap.parse_args()
    globals()["MODEL"] = a_.model
    sfx = "" if a_.model == "mamba4" else f"_{a_.model}"

    OUT.mkdir(parents=True, exist_ok=True)
    rows = [r for r in csv.DictReader(open(S / "model_vs_ribo.tsv"), delimiter="\t")
            if r["model"] == MODEL]
    # THE DELTAS ARE NOW FILTERED BY MODEL TOO. They were read unfiltered while the heatmap above
    # was mamba4-only, so one figure mixed a single-architecture grid with a two-architecture
    # summary statistic. It mattered: pooled gives mean +0.0035, range -0.0042..+0.0070, negative in
    # 2 of 12 -- but BOTH negatives are mamba4. For attn the effect is +0.0041, range
    # +0.0030..+0.0053, negative in 0 of 6, i.e. matched RNA never hurts.
    ...
    for ext in ("pdf", "png"):
        fig.savefig(OUT / f"E1_rna_provenance{sfx}.{ext}", dpi=200, bbox_inches="tight")

    (OUT / f"E1_values{sfx}.json").write_text(json.dumps(
        dict(model=MODEL, datasets=DS, n_rna_samples=NSAMP, grids=grids,
             ...
             n_deltas=len(deltas),
             n_deltas_negative=sum(1 for d in deltas if d < 0),
             aggregation=("deltas are matched-minus-mismatched F1, filtered to this model "
                          "(3 ribo references x 2 arms = 6). They were previously read UNFILTERED "
                          "(12, both architectures) while the grid was single-model.")),
        indent=2))
    with open(OUT / f"E1_values{sfx}.tsv", "w") as fh:
        fh.write("arm\trna_input\tribo_reference\tf1\tmatched\n")
        for arm, _ in arms:
            for i, rna in enumerate(DS):
                for j, ref in enumerate(DS):
                    fh.write(f"{arm}\t{rna}\t{ref}\t{grids[arm][i][j]:.4f}\t{int(i == j)}\n")
    print(f"  wrote {OUT}/E1_rna_provenance{sfx}.{{pdf,png}}   model={MODEL}")
