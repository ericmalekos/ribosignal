...
# PREFER THE FINAL-RECIPE DUMP, matching P5. The hepatocyte arm has two: `dropin/` (built on
# data/packed_union, mm1) and `dropin_canonpack/` (re-dumped against packed_canon_Hepatocytes for
# task #86). Consumers were previously establishing that A2 and P5's hepatocyte group came from the
# same run by comparing floats to 1e-12, because neither file recorded its model or substrate.
_CANON = RUN / "dropin_canonpack/dropin_metrics.json"
SRC = _CANON if _CANON.exists() else RUN / "dropin/dropin_metrics.json"
ON_FINAL_RECIPE = SRC.parent.name == "dropin_canonpack"
...
def check_checkpoint():
    npz = SRC.parent / "pred_profiles.npz"
    if not npz.exists():
...
def main():
    ...
    # PROVENANCE IS RECORDED, not inferred. This file carried neither model nor substrate, so a
    # consumer had to prove it matched P5's hepatocyte group by comparing floats to 1e-12.
    (HERE / f"A2_values{SUF}.json").write_text(json.dumps(
        {"model": MODEL,
         "on_final_recipe": ON_FINAL_RECIPE,
         "pack": ("data/packed_canon_Hepatocytes" if ON_FINAL_RECIPE
                  else "data/packed_union (mm1)"),
         "source": str(SRC),
         "arm": "pred_preddepth",
