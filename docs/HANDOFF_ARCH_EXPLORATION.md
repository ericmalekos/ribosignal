...
- **mamba_ssm is CUDA-only** (`causal_conv1d_fwd` asserts `x.is_cuda`) — no CPU fallback, even for
  a smoke test. SIF: `singularity_cache/ghcr.io-ericmalekos-rnazoo-orthrus-latest.img`.
